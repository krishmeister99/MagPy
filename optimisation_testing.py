...
system_params = SystemParams(J = 1, # Exchange Stiffness
                             ...
                             L = 20 # System Length (in units of 'a')
                             )
...
def dM_dt(t,M):
    ...
    '''
    H_cross = sk.H_matrix(M, system_params, t)
    xprod1 = H_cross.dot(M)
    M_cross = sk.M_matrix(M)
...
dt = 0.05
i = 0
Data = np.zeros(shape=( np.int(3*(system_params.array_size**2)) , np.int(tf/dt)))
...
anim = animation.FuncAnimation(fig, update_stuff, frames = np.int(tf/dt),  fargs=(Q, I, X, Y, time_text),
