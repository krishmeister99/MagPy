...
def H_eff(M , i , j , system_params, t):
    ...
    # Zeeman term:
    # Adding a sinusoidal out of plane magnetic field:
    zee = Z*B*np.sin(omega*t)
    # Uniaxial anisotropy:
...
# H_matrix and M_matrix functions with COO sparse format:
def H_matrix_coo(M , system_params , t):
    """
    Achieves the same result as H_matrix, however the sparse format is COO.
    """
    # Width of the system:
    N = np.int(np.sqrt(len(M)/3))
    # Generating H_eff array.
    h = np.zeros(shape = (3*N**2))
    for i in range(N):
        for j in range(N):
            h[3*(N*i + j) : 3*(N*i + j) + 3] = H_eff(M, i , j, system_params, t)
    ...
    return sparse.coo_matrix((h_shuffled , (rows , columns)), shape = (3*N**2 , 3*N**2)) - sparse.coo_matrix((h_shuffled , (columns , rows)), shape = (3*N**2 , 3*N**2))
...
def M_matrix_coo(M):
    ...
    block = np.array([[0,0,-1],[0 , 1 , 0],[-1  , 0 , 0]])
    # Shuffling around elements of h, so that the resulting matrix has the correct cross product structure
    M_shuffled = sparse.kron(sparse.identity(N**2) , block)@M

    return sparse.coo_matrix((M_shuffled , (rows , columns)), shape = (3*N**2 , 3*N**2)) - sparse.coo_matrix((M_shuffled , (columns , rows)), shape = (3*N**2 , 3*N**2))
